...
get_worktree_path() {
    ...
    # Also check via git worktree list in case of non-standard paths
    if has_git; then
        local wt_path=""
        local current_path=""
...
# List all spec directories across all worktrees and the main repo
# Output format: one spec dir path per line
list_all_specs() {
    local repo_root=$(get_repo_root)
